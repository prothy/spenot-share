...
export async function saveUser(req) {
     ...
     * @returns {Array} status - Returns an array with HTTP response code and message values respectively. 
     */
    ...
    if (await User.findOne({ email })) {
        return [500, 'E-mail already exists. Use another one.']
    }
    if (await User.findOne({ username })) {
        return [500, 'Username already exists. Use another one.']
    }
