...
router.get('/api/song/:songId', async (req, res) => {
    ...
    await spotifyHelper.getSongById(req.params.songId, req.session.accessToken).catch(e => console.error(`Error getting song by id (${JSON.stringify(e.response.data.error)})`))
    // console.log(songInfo)
    await spotifyHelper.addSongToPlaylist(req.session.playlistId, req.session.accessToken, req.params.songId).catch(e => console.error('Error adding song to playlist: ' + e))
